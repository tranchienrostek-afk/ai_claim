...
async function safeWrite(db) {
  ...
  try {
    // Acquire file lock (cross-process serialization)
    release = await lockfile.lock(DB_FILE, LOCK_OPTIONS);
    // On Windows, the atomic rename performed by lowdb/steno can transiently
    // fail with EPERM if AV / Search indexer / OneDrive briefly holds the
    // destination file. Retry a few times with a short backoff before giving up.
    let attempt = 0;
    const maxAttempts = 5;
    ...
    while (true) {
      try {
        ...
        if (writeErr && writeErr.code === "EPERM" && attempt < maxAttempts - 1) {
          attempt += 1;
          const backoffMs = 25 * Math.pow(2, attempt - 1);
          console.warn(`[DB] EPERM on rename (attempt ${attempt}/${maxAttempts}), retrying in ${backoffMs}ms...`);
